...
class LiveAnimalDetector:
    ...
    def send_alert(self, detections, cctv_id, video_path=None):
        try:
            ...
            r = requests.post(f"{self.backend_url}/api/wildDetection/detections",
                              json=payload, timeout=30)
            if r.status_code == 201:
                logger.info("Alert sent to backend successfully")
                self.stats["alerts_sent"] += 1
            else:
                logger.warning(f"Backend responded with status {r.status_code} - {r.text}")
    ...
    def detect(self, camera_index=0, cctv_id="cam_001", show=True):
        try:
            ...
            while True:
                ...
                resized = cv2.resize(frame, (320, 256))
                results = self.model.track(resized, conf=self.confidence_threshold,
                ...
                if results[0].boxes is not None:
                    for box in results[0].boxes:
                        ...
                        cv2.rectangle(resized, (x1, y1), (x2, y2), color, 2)
                        cv2.putText(resized, f"{class_name} ID:{track_id}", (x1, y1 - 10),
                ...
                if alert and len(valid_detections) > 0:
                    logger.warning("Animal detected — sending alert")
                    clip_path = self.capture_clip(frame_buffer)
                    self.send_alert(valid_detections, cctv_id, clip_path)
