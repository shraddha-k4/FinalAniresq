...
"""
AniResQ Camera Detection Client with Image Saving, Location, and MongoDB
"""
...
DB_NAME = os.getenv("MONGO_DB_NAME", "aniresq_images")
SAVE_LOCAL = True  # Whether to save detected frames locally
LOCAL_SAVE_DIR = os.getenv("LOCAL_SAVE_DIR", "./detections")  # Folder for images
...
fs = gridfs.GridFS(db)
images_collection = db['detections']

# Ensure local directory exists
if SAVE_LOCAL and not os.path.exists(LOCAL_SAVE_DIR):
...
class CameraDetectionClient:
    ...
    def send_frame_to_ml(self, frame_base64, confidence=0.5):
        try:
            payload = {'frame': f'data:image/jpeg;base64,{frame_base64}', 'confidence': confidence}
            response = requests.post(f'{self.ml_service_url}/api/detect/frame', json=payload, timeout=10)
            if response.status_code == 200:
                return response.json()
            logger.error(f"❌ ML service error {response.status_code}")
            return None
    ...
    def save_detection(self, frame, detections, timestamp):
        ...
        logger.info(f"💾 Saved to MongoDB: {filename} (lat: {lat}, lon: {lon})")
    ...
    def process_detection_result(self, frame, result):
        ...
        if detections:
            self.detection_count += len(detections)
            logger.warning(f"🚨 DETECTION ALERT! Found {len(detections)} animal(s)")
            for det in detections:
                logger.warning(f"🐾 {det['class_name']} - Confidence {det['confidence']:.2%}")

            # Save frame with detections
            self.save_detection(frame, detections, datetime.utcnow())

    def process_frames(self, duration_seconds=60, skip_frames=2, confidence=0.5):
        logger.info(f"🎥 Starting camera capture for {duration_seconds} seconds...")
        self.running = True
        start_time = datetime.now()
        skip_counter = 0
        ...
        try:
            while self.running:
                ...
                if frame_base64:
                    result = self.send_frame_to_ml(frame_base64, confidence)
                    self.process_detection_result(frame_resized, result)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='AniResQ Camera Client with Location')
    parser.add_argument('--ml-service', default='http://localhost:5000')
    parser.add_argument('--camera', type=int, default=0)
    ...
    if client.connect_camera():
        client.process_frames(duration_seconds=args.duration, skip_frames=args.skip_frames, confidence=args.confidence)
    else:
